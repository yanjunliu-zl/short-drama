...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const handleMenuClick = ({ key }: { key: string }) => {
    ...
    // 需要登录的页面：未登录时跳转到登录页
    const protectedPages = ['story_script', 'scene_character_props', 'storyboard_script', 'render', 'final_video'];
    ...
    if (protectedPages.includes(key)) {
      const workId = localStorage.getItem(`pipeline_cache_${reduxUserId}_workId`) || localStorage.getItem(`pipeline_${reduxUserId}_workId`);
