    ...
    if (editingShot) {
      ...
      if (editingShot.id) {
        setShots(shots.map(shot =>
          shot.id === editingShot.id ? updatedShot : shot
        ));
        message.success('分镜头已更新');
      } else {
        setShots([...shots, updatedShot]);
        message.success('分镜头已添加');
      }
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          ...
          <Text type="secondary">总时长：{formatDuration(totalDuration)}，共 {shots.length} 个镜头</Text>
        </div>
        <Space>
        ...
        <div style={{ display: 'flex', overflowX: 'auto', padding: '8px 0' }}>
          {shots.map((shot, index) => (
            ...
            >
              ...
              >
                {index === 0 ? '00:00' : `${Math.floor(shots.slice(0, index).reduce((sum, s) => sum + s.duration, 0) / 60)}:${(shots.slice(0, index).reduce((sum, s) => sum + s.duration, 0) % 60).toString().padStart(2, '0')}`}
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <Text type="secondary">设计和编辑视频的分镜头脚本，包括画面、对白、音效等元素</Text>
      </div>
