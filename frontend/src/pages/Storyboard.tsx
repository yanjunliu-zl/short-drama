...
const Storyboard: React.FC = () => {
  ...
  const collab = useCollaboration(searchParams.get('workId'), 'storyboard');
  const hasWorkId = !!(searchParams.get('workId')?.startsWith('wk_') || getWorkId());
  // 多集数据状态
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      let data: any = await loadState('storyboard', validWorkId || undefined);

      // Load episodes from script pipeline if storyboard data is absent
      if (!data || !data.episodes || data.episodes.length === 0) {
        const scriptData = await loadState('script');
      ...
      // Load reference images from pipeline hook (backend-first, cache fallback)
      const refData = await loadState('referenceImages', validWorkId || undefined);
      if (refData) setReferenceImages(refData);
    };
    loadData();
  }, [searchParams]);
        ...
        if (status) {
          ...
          if (status.status === 'completed') {
            ...
            if (result.results && episodes.length > 0) {
              ...
              // Persist to backend via pipeline hook (source of truth, cache synced automatically)
              const videoData = {
              ...
              const wId = getWorkId();
              saveState('videoResults', videoData, wId || undefined);
              saveState('storyboard', { episodes: updatedEpisodes, generatedAt: new Date().toISOString() }, wId || undefined);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {collab.hasRemoteChanges && (
            <Tooltip title="检测到远程更新">
              <Button size="small" type="link" danger onClick={collab.syncFromRemote}>⚡ 有新版本</Button>
            </Tooltip>
          )}
        </div>
        <Button
      ...
      {/* 无作品提示 */}
      {!hasWorkId && (
        <div style={{ padding: '10px 24px', background: '#fffbe6', borderBottom: '1px solid #ffe58f', textAlign: 'center' }}>
