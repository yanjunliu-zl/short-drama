...
export const scriptService = {
  ...
  generateScriptFromOutlineSync: async (data: {
    ...
    const response = await api.post<import('@/types').ScriptSplitResponse>('/v1/scripts/generate/from-outline', data, { timeout: 600000 })
    return response.data
  },
  ...
  /** 记录用户在多版本对比中选择了哪个版本，积累偏好数据 */
  recordVersionChoice: async (data: {
    user_id: string
    chosen_version: string
    versions: Array<{ version: string; label: string; score: number }>
    style?: string
    theme?: string
    length?: string
    auto_choice?: boolean
  }): Promise<ApiResponse<{ angles: Record<string, number>; preferred_angle: string; total_scripts: number }>> => {
    const response = await api.post('/v1/scripts/preferences/version-choice', data)
    return response.data
  },

  /** 查询用户的叙事角度偏好积累数据 */
  getAnglePreferences: async (userId: string): Promise<{
    angles: Record<string, number>
    preferred_angle: string
    total_choices: number
    angle_history: Array<{ angle: string; label: string; score: number }>
  }> => {
    const response = await api.get('/v1/scripts/preferences/angles', { params: { user_id: userId } })
    return response.data
  },

  // ── 对话式意图挖掘 ──

  /** 在生成前挖掘用户的真实意图，返回澄清问题或丰富后的大纲 */
  mineIntent: async (data: {
    idea: string
    style?: string
    theme?: string
    length?: string
    ...
    enriched_outline: string
    questions: Array<{ id: string; question: string; options: string[] }>
    detected_genre: string
    suggested_style: string
    suggested_length: string
  }> => {
    const response = await api.post('/v1/scripts/generate/intent', data)
    return response.data
  },
  ...
  generateShotsVideo: async (data: {
    ...
    const response = await api.post('/v1/render/shots-to-video', data)
    return response.data
  },
  ...
  getShotsVideoStatus: async (taskId: string): Promise<{
    ...
    const response = await api.get(`/v1/render/shots-to-video/${taskId}/status`)
    return response.data
  },
  ...
  getShotsVideoResult: async (taskId: string): Promise<{
    ...
    const response = await api.get(`/v1/render/shots-to-video/${taskId}`)
    return response.data
  },
  ...
  getPreviewImageStatus: async (taskId: string): Promise<{
    ...
    const response = await api.get(`/v1/render/preview-image/${taskId}/status`)
    return response.data
  },
}