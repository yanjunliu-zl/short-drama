...
docker compose up -d script-service storyboard-service render-service
...
```bash
# Scene image
curl -X POST http://localhost/api/v1/render/images/generate \
  -d '{"scene_description":"Ancient palace interior","storyboard_id":"sb-1","scene_number":1,"style":"Ancient"}'

# Image to video
curl -X POST http://localhost/api/v1/render/videos/generate \
  -d '{"image_url":"http://...","prompt":"Slow camera push-in","duration":5.0}'

# Batch shots to video
curl -X POST http://localhost/api/v1/render/shots-to-video \
