...
//
// [DEPRECATED] This gRPC server currently has zero clients.
// All inter-service communication goes through REST/APISIX.
//
// If gRPC is not needed, remove this package and:
//   - Delete backend/proto/ directory
//   - Remove grpc-related dependencies from go.mod
//   - Remove gRPC port from k8s service definitions
//
// If gRPC IS needed in the future:
//   1. Enable otelgrpc interceptor for distributed tracing
//   2. Enable grpc_prometheus for metrics
//   3. Create gRPC clients in services that need to call FinalCut
//   4. Add gRPC routes to APISIX (or use gRPC load balancing)
//
