"""Prometheus 指标中间件 — llmhua-service"""
import time
import logging
...
# 业务指标
IMAGE_GENERATION_COUNT = Counter("image_generation_total", "Image generations", ["status"], registry=registry)
IMAGE_GENERATION_DURATION = Histogram("image_generation_duration_seconds", "Image generation duration", buckets=[5, 15, 30, 60, 120, float("inf")], registry=registry)
VIDEO_GENERATION_COUNT = Counter("video_generation_total", "Video generations", ["status"], registry=registry)
VIDEO_GENERATION_DURATION = Histogram("video_generation_duration_seconds", "Video generation duration", buckets=[10, 30, 60, 120, 300, float("inf")], registry=registry)
HEALTH_STATUS = Gauge("service_health_status", "Health status", ["service"], registry=registry)


class PrometheusMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, app_name: str = "llmhua-service"):
        super().__init__(app)
...
def setup_metrics(app: FastAPI, app_name: str = "llmhua-service"):
    """注册 /metrics 端点和中间件"""
