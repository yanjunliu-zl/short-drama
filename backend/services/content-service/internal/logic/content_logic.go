...
func (l *ContentLogic) SavePipelineState(ctx context.Context, req *types.SavePipelineStateRequest) (*types.PipelineStateResponse, error) {
	...
	// Sync to Redis hash (for per-step reads)
	pk := fmt.Sprintf("pipeline:%s", req.WorkID)
	for k, v := range existing {
		if b, err := json.Marshal(v); err == nil {
			l.redis.Hset(pk, k, string(b))
		}
	}
...
// GetPipelineState 加载管道状态 JSON 快照 — Redis 优先，MySQL fallback
func (l *ContentLogic) GetPipelineState(ctx context.Context, req *types.GetPipelineStateRequest) (*types.PipelineStateResponse, error) {
	...
	pk := fmt.Sprintf("pipeline:%s", req.WorkID)
	all, err := l.redis.Hgetall(pk)
	if err == nil && len(all) > 0 {
		// Redis hit — reconstruct PipelineState from hash
		...
		for k, v := range all {
			...
			case "videoResults":
				state.VideoResults = parsed
			case "finalCut":
				state.FinalCut = parsed
			case "referenceImages":
				state.ReferenceImages = parsed
			}
		}
		logx.WithContext(ctx).Infof("[ContentLogic] GetPipelineState redis hit workID=%s keys=%d", req.WorkID, len(all))
		return &types.PipelineStateResponse{WorkID: req.WorkID, Data: state}, nil
	}

	// Redis miss — fallback to MySQL
	dataStr, err := l.repo.GetPipelineData(ctx, req.WorkID)
	if err != nil {
		logx.WithContext(ctx).Errorf("[ContentLogic] GetPipelineState mysql failed workID=%s: %v", req.WorkID, err)
		return nil, err
	}
	if dataStr == "" {
		return &types.PipelineStateResponse{WorkID: req.WorkID, Data: nil}, nil
	}
...
// ==============================
// Per-step pipeline — Redis hash atomic reads/writes
// ==============================

// SavePipelineStep atomically writes a single step to Redis hash (async MySQL backup)
func (l *ContentLogic) SavePipelineStep(ctx context.Context, req *types.SavePipelineStepRequest) (*types.PipelineStepResponse, error) {
	logx.WithContext(ctx).Infof("[ContentLogic] SavePipelineStep workID=%s step=%s", req.WorkID, req.Step)
	...
	pk := fmt.Sprintf("pipeline:%s", req.WorkID)
	if err := l.redis.Hset(pk, req.Step, string(dataBytes)); err != nil {
		logx.WithContext(ctx).Errorf("[ContentLogic] SavePipelineStep redis hset failed: %v", err)
		return nil, fmt.Errorf("save pipeline step: %w", err)
	}

	// Async MySQL backup (best-effort, non-blocking)
	go func() {
		all, err := l.redis.Hgetall(pk)
		if err != nil {
			return
		...
		full["updatedAt"] = time.Now().UTC().Format(time.RFC3339)
		if raw, err := json.Marshal(full); err == nil {
			l.repo.SavePipelineData(context.Background(), req.WorkID, string(raw))
		}
	}()

	return &types.PipelineStepResponse{
		WorkID: req.WorkID,
		Step:   req.Step,
		Data:   req.Data,
	}, nil
...
// GetPipelineStep reads a single step from Redis, falls back to MySQL on miss
func (l *ContentLogic) GetPipelineStep(ctx context.Context, req *types.GetPipelineStepRequest) (*types.PipelineStepResponse, error) {
	logx.WithContext(ctx).Infof("[ContentLogic] GetPipelineStep workID=%s step=%s", req.WorkID, req.Step)

	pk := fmt.Sprintf("pipeline:%s", req.WorkID)
	raw, err := l.redis.Hget(pk, req.Step)
	if err != nil || raw == "" {
		if raw == "" {
			logx.WithContext(ctx).Infof("[ContentLogic] GetPipelineStep redis miss, falling back to MySQL")
		}
		fullStr, mysqlErr := l.repo.GetPipelineData(ctx, req.WorkID)
		if mysqlErr != nil || fullStr == "" {
			return &types.PipelineStepResponse{WorkID: req.WorkID, Step: req.Step, Data: nil}, nil
		}
		var full map[string]interface{}
		if err := json.Unmarshal([]byte(fullStr), &full); err == nil {
			for k, v := range full {
				if b, err := json.Marshal(v); err == nil {
					l.redis.Hset(pk, k, string(b))
				}
			}
			if v, ok := full[req.Step]; ok {
				return &types.PipelineStepResponse{WorkID: req.WorkID, Step: req.Step, Data: v}, nil
			}
		}
		return &types.PipelineStepResponse{WorkID: req.WorkID, Step: req.Step, Data: nil}, nil
	}
	...
	if err := json.Unmarshal([]byte(raw), &data); err != nil {
		return nil, fmt.Errorf("unmarshal step data: %w", err)
	}
	return &types.PipelineStepResponse{
		WorkID: req.WorkID,
		Step:   req.Step,
