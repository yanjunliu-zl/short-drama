	...
	server.AddRoutes(
		[]rest.Route{
			...
			},
			{
				...
				{
					Method:  http.MethodPut,
					Path:    "/api/v1/works/:id/pipeline/:step",
					Handler: SavePipelineStepHandler(serverCtx),
				},
				{
					Method:  http.MethodGet,
					Path:    "/api/v1/works/:id/pipeline/:step",
					Handler: GetPipelineStepHandler(serverCtx),
			},
		},
	)
		...
		[]rest.Route{
			{
				...
				Handler: GetCreditBalanceHandler(serverCtx),
			},
			{
				Method:  http.MethodPost,
				Path:    "/api/v1/credits/deduct",
				Handler: DeductCreditHandler(serverCtx),
			},
		},
	)
