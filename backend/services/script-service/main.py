...
async def startup_event():
    ...
    try:
        ...
        await worker.start(
            ...
            concurrency=settings.KAFKA_CONSUMER_CONCURRENCY,
        )
        logger.info(f"Task worker started: backend={settings.TASK_QUEUE_BACKEND}")
    except Exception as e:
        logger.warning(f"Task worker startup skipped ({e}) — worker will init lazily on first request")
