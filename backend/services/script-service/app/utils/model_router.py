...
def create_llm_for_task(
    ...
    """Create an LLM client with task-aware tier routing.

    Uses shared/model_tiers.route_by_task() to select the appropriate
    model tier based on task complexity (script length, novel vs outline, etc.),
    then creates the best client for that tier.

    Tier mapping:
      - TINY/SMALL → vLLM self-hosted (zero cost, lowest latency)
      - LARGE      → vLLM 14B or DeepSeek
      - CLOUD      → DeepSeek / OpenAI / Anthropic (highest quality)

    Args:
        request: The generation request dict (must contain 'length' and optionally
                 'novel_content' / 'outline').
        timeout: HTTP timeout in seconds.

    Returns:
        A ResilientLLM or None if no provider is available.
    """
    try:
        # shared/python/model_tiers.py — copied by Dockerfile into app/services/
        from app.services.model_tiers import route_by_task, ModelTier  # noqa: F811
    except ImportError:
        # model_tiers not in PYTHONPATH — use locale-aware routing as fallback
        target_locale = request.get("target_locale", "zh-CN")
        return create_llm_for_locale(target_locale=target_locale, timeout=timeout)

    tier = route_by_task(request)
    logger.info(f"ModelRouter: task-aware routing → tier={tier} "
                f"(length={request.get('length')}, "
                f"novel_len={len(request.get('novel_content', ''))}, "
                f"has_outline={bool(request.get('outline', ''))})")

    if tier == ModelTier.TINY or tier == ModelTier.SMALL:
        # Try vLLM self-hosted first, fall back to cloud
        client = _try_create_vllm(timeout=timeout)
        if client:
            global _active_provider
            _active_provider = "vllm"
            return ResilientLLM(client, provider_name="vllm", timeout=timeout)
        logger.info(f"ModelRouter: tier={tier} vLLM unavailable, falling back to cloud")

    # LARGE or CLOUD tier, or vLLM fallback
    return create_llm_client(prefer="deepseek", timeout=timeout)
