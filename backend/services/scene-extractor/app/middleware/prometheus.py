"""Prometheus 指标中间件 — scene-extractor"""
import time
import logging
...
# 业务指标
SCENE_EXTRACTION_COUNT = Counter("scene_extraction_total", "Scene extractions", ["type", "status"], registry=registry)
SCENE_EXTRACTION_DURATION = Histogram("scene_extraction_duration_seconds", "Extraction duration", ["type"], buckets=[1, 5, 15, 30, 60, float("inf")], registry=registry)
HEALTH_STATUS = Gauge("service_health_status", "Health status", ["service"], registry=registry)


class PrometheusMiddleware(BaseHTTPMiddleware):
    def __init__(self, app: ASGIApp, app_name: str = "scene-extractor"):
        super().__init__(app)
...
def setup_metrics(app: FastAPI, app_name: str = "scene-extractor"):
    """注册 /metrics 端点和中间件"""
