...
async def generate_shots_to_video(
    ...
    if use_streaming:
        ...
        # Build character consistency context from reference images
        ref_chars = {}
        ref_multiview: dict = {}
        if request.referenceImages and request.referenceImages.characters:
            ref_chars = request.referenceImages.characters
            for name, url in ref_chars.items():
                if name.endswith('_侧脸') or name.endswith('_3/4'):
                    continue
                ref_multiview[name] = {"front": url}
                side_key = f"{name}_侧脸"
                threeq_key = f"{name}_3/4"
                if side_key in ref_chars:
                    ref_multiview[name]["side"] = ref_chars[side_key]
                if threeq_key in ref_chars:
                    ref_multiview[name]["threeQuarter"] = ref_chars[threeq_key]

        # Per-character seed map for cross-shot facial consistency
        char_seeds_ss: dict = {}

        async def generate_shot_image(shot, ep_idx: int):
            """Generate image+video for a single shot with character-anchored seed + retry."""
            img_prompt = (
                shot.imagePromptZh or shot.imagePrompt or
                shot.description
            )

            # Character-anchored seed
            chars = shot.characters or []
            primary_char = chars[0] if chars else ''
            seed_key = f"{primary_char}:{shot.sceneRef or ''}"
            if seed_key not in char_seeds_ss:
                char_seeds_ss[seed_key] = abs(hash(seed_key)) % (2**31)
            char_seed = char_seeds_ss[seed_key]

            # Character consistency prompt injection
            if ref_chars and chars:
                char_hints = []
                for c in chars[:3]:
                    if c in ref_chars:
                        char_hints.append(f"角色「{c}」面部特征与参考图保持一致")
                        if c in ref_multiview and len(ref_multiview[c]) >= 3:
                            char_hints.append(f"「{c}」多角度参考已提供")
                if char_hints:
                    img_prompt = f"{img_prompt}。【角色一致性锁定】{'；'.join(char_hints)}。角色面部、发型、肤色、体型必须与参考完全相同"
            ...
            video_url = None
            if img_result and img_result.get("image_url"):
                vid_prompt = _build_shot_video_prompt(shot.model_dump() if hasattr(shot, 'model_dump') else shot, request.style or "写实风格")
...
async def _generate_shots_to_video_task(
    ...
    try:
        ...
        video_cost_cents = total_shots * 250  # ~2.5元/镜头 (5s × 0.5元/s)
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                check = await client.get(
                    f"http://content-service:8081/api/v1/credits/balance?userId={user_id}"
                )
                if check.status_code == 200:
                    bal = check.json()
                    if bal.get("balance", 0) < video_cost_cents:
                        await task_store.update(task_id, {
                            "status": "failed",
                            "error": f"余额不足：当前 ¥{bal.get('balanceYuan', 0):.2f}，需要 ¥{video_cost_cents / 100:.2f}",
                        })
                        logger.warning(f"[Task] 余额不足 user={user_id} need={video_cost_cents/100:.2f}")
                        return
        except Exception as e:
        ...
        failed_count = 0
        cached_count = 0
        # Per-character + scene anchor seeds: same character in same scene → same seed → consistent face
        char_seeds: dict = {}
        # Collect character multi-angle references from request
        ref_multiview: dict = {}  # char_name → {front, side, threeQuarter} urls
        if request.referenceImages and request.referenceImages.characters:
            for name, url in request.referenceImages.characters.items():
                if name.endswith('_侧脸') or name.endswith('_3/4'):
                    continue  # sub-views handled by their base name entry
                ref_multiview[name] = {"front": url}
                side_key = f"{name}_侧脸"
                threeq_key = f"{name}_3/4"
                if side_key in request.referenceImages.characters:
                    ref_multiview[name]["side"] = request.referenceImages.characters[side_key]
                if threeq_key in request.referenceImages.characters:
                    ref_multiview[name]["threeQuarter"] = request.referenceImages.characters[threeq_key]

        def _get_char_seed(char_name: str, scene_ref: str) -> int:
            """Generate a stable, deterministic seed for a character in a specific scene."""
            key = f"{char_name}:{scene_ref}"
            if key not in char_seeds:
                char_seeds[key] = abs(hash(key)) % (2**31)
            return char_seeds[key]

        def _build_char_consistency_prompt(characters: list, ref_chars: dict, ref_mv: dict) -> str:
            """Build character consistency segment using reference images and multi-angle hints."""
            if not characters or not ref_chars:
                return ""
            parts = []
            for ch in characters[:3]:
                if ch in ref_chars:
                    parts.append(f"角色「{ch}」面部特征与参考图保持一致")
                    if ch in ref_mv and len(ref_mv[ch]) >= 3:
                        parts.append(f"「{ch}」多角度参考已提供(正面+侧面+3/4)")
            if parts:
                return "。【角色一致性锁定】" + "；".join(parts) + "。所有角色面部结构、发型、肤色、体型必须与参考完全相同，禁止自由发挥"
            return ""
        ...
        async def _get_cached_result(shot_dict: dict, style: str, model: str) -> dict | None:
            ...
            try:
                ...
                raw = await store.redis.get(cache_key)
                if raw:
        ...
        async def _set_cached_result(shot_dict: dict, style: str, model: str, result: dict):
            ...
            try:
                store = await get_task_store()
                await store.redis.setex(cache_key, 172800, json.dumps(result))
            except Exception:
                pass
        ...
        for ep in request.episodes:
            for shot in ep.shots:
                ...
                # ── 缓存检查：相同镜头参数不再重复生成，节省 API 成本 ──
                cached = await _get_cached_result(shot_dict, request.style or "", request.model or "seedance")
                if cached:
                    logger.info(f"镜头 {shot_dict.get('number','?')} 缓存命中，跳过生成")
                    cached_count += 1
                ...
                try:
                    ...
                    # ── Character Consistency (S11 enhanced) ──
                    chars = shot_dict.get('characters', []) or []
                    ref_chars = request.referenceImages.characters if request.referenceImages else {}
                    scene_ref = shot_dict.get('sceneRef', '')

                    # Character-anchored seed: same character + scene → same seed → consistent facial features
                    primary_char = chars[0] if chars else ''
                    if primary_char and request.characterLock:
                        shared_seed = _get_char_seed(primary_char, scene_ref)
                        logger.debug(f"镜头 {shot_dict.get('number','?')}: seed={shared_seed} (char={primary_char}, scene={scene_ref})")
                    else:
                ...
                # 缓存成功的结果，48h 内相同参数不重复调用 API
                if status == "completed" and video_url:
                    await _set_cached_result(shot_dict, request.style or "", request.model or "seedance",
                        {"video_url": video_url, "image_url": image_url, "file_size": file_size})
        ...
        # ── 扣减信用额度 ──
        actual_cost = completed_count * 250  # 只按成功数扣费
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                resp = await client.post(
                    "http://content-service:8081/api/v1/credits/deduct",
                    json={"userId": user_id, "amount": actual_cost},
                )
                if resp.status_code == 200:
                    data = resp.json()
                    logger.info(f"[Billing] 扣费成功 user={user_id} amount=¥{actual_cost/100:.2f} remaining=¥{data.get('balanceYuan',0):.2f}")
        except Exception as e:
            logger.error(f"[Billing] 扣费失败 user={user_id}: {e}")
...
async def health_check():
    ...
    return {
        "status": "healthy",
        "service": "render-video-generation",
        "timestamp": time.time()
    }
